    ...
    <>
      {
        items.map((item, index) => {
          ...
          return (
            <Link href={item.comingSoon ? '' : item.href} key={index} locale={item.locale ? item.locale : ''}>
              <a 
              ...
              >
                <div className={`flex-shrink-0 flex items-center justify-center h-10 w-10 rounded-lg ${item.className ? item.className : item.locale ? '' : 'border border-gray-800 bg-secondary-100'} text-gray-900 sm:h-12 sm:w-12 ${item.comingSoon && 'opacity-50'}`}>
